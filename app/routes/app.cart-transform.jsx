...
export const loader = async ({ request }) => {
  ...
  // Check if cartTransform is registered on the store
  ...
  try {
    ...
    const nodes = data?.data?.cartTransforms?.nodes || [];
    if (nodes.length > 0) {
      activeTransform = nodes[0];
    }
  } catch (err) {
    console.error("Error fetching cart transforms:", err);
  }
...
export default function CartTransformPage() {
  const { activeTransform } = useLoaderData();
        ...
        <s-button
          ...
          onClick={() => {
            if (isFunctionActive) {
              ...
              fetcher.submit(
                { intent: "activate", functionId: "cart-transform" },
                { method: "POST" }
              );
            }
          }}
