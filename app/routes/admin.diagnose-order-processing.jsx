...
export const loader = async ({ request }) => {
  ...
  const receiptWhere = {};
  if (orderId) receiptWhere.orderId = String(orderId);
  if (topic) receiptWhere.topic = String(topic);

  const [webhookReceipts, waNotifications, emailNotifications] = await Promise.all([
    db.webhookReceiptLog.findMany({ where: receiptWhere, orderBy: { receivedAt: "desc" }, take: 30 }),
  ...
  return Response.json({
    ...
    gmail,
    filter: { orderId: orderId || null, topic: topic || null },
    webhookReceipts,
    waNotifications,
    emailNotifications,
  });
