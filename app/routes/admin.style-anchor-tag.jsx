/**
 * TEMPORARY one-time fix — strips the default themed link look (colour +
 * underline) off the "Blue Sapphire" anchor in Blue Sapphire's
 * tab_1_content, replacing it with inline styling so it reads as normal
 * body text but semi-bold, scoped to just this one link (not a
 * sitewide/tab-wide CSS change). Delete once run.
 *
 *   GET /admin/style-anchor-tag?secret=<MIGRATION_SECRET>&handle=blue-sapphire&key=tab_1_content
 */
...
const BEFORE_TAG = '<a href="/collections/blue-sapphire">Blue Sapphire</a>';
const AFTER_TAG =
  '<a href="/collections/blue-sapphire" style="color: inherit; text-decoration: none; font-weight: 600;">Blue Sapphire</a>';
...
export const loader = async ({ request }) => {
  ...
  try {
    ...
    if (!before.includes(BEFORE_TAG)) {
      return Response.json({ ok: true, message: "Expected tag not found, nothing to change.", currentValue: before });
    }
    const after = before.replace(BEFORE_TAG, AFTER_TAG);
