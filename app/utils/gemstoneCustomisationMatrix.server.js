...
export function generateAllCustomisationVariants(rates) {
  ...
  // Pearl BEFORE default, and pearl designs get their own distinct
  // "<code> (Pearl)" option value instead of sharing the bare code
  // ("RD01", "RD02", ...) with the default catalog. Two real bugs this
  // fixes together, both pearl-only:
  //  1. `default` and `pearl` reuse the exact same design codes for the
  //     same Type/Metal (both define Ring/Silver/RD01, etc., just with
  //     different weights). A Shopify variant is only ever identified by
  //     its Type+Metal+Design combo, so without a distinguishing suffix
  //     these collided in `seenCombinations` -- whichever set was
  //     processed first (`default`) silently "won" that combo, and every
  //     pearl product customizing with a code that also exists in
  //     `default` got charged `default`'s price/weight instead of
  //     pearl's, with no visible error.
  //  2. Liquid's `product.variants` (the storefront's fallback lookup
  //     when the app-pushed `_shubhCustomisationMatrix` snippet is
  //     empty -- see shubh-gems-customizer.liquid, it always has been so
  //     far) only ever sees the first 250 variants, and this catalog
  //     already has 260+. Whichever set is appended SECOND risks being
  //     pushed past that cap and going completely invisible to the
  //     theme. Putting pearl first (right after the unshifted Utility
  //     variant below) protects it the same way that variant already
  //     protects itself.
  for (const setKey of ["pearl", "default"]) {
  ...
  // Dedicated "surcharge unit" variant, priced at exactly Rs 1 -- the
  // ONLY reliable target for the custom-design ("Customised"/uploaded
  // photo) fallback in submitCustomizerBundle(), which can't look up a
  // real price (there's no catalog entry for a photo that doesn't exist
  // yet) and instead sets cart quantity = the intended surcharge amount,
  // relying on quantity x unit price = the real total. That trick is
  // only accurate when unit price is Rs 1; before this, the fallback
  // pointed at whatever the FIRST real variant in this array happened to
  // be (an arbitrary actual design, e.g. "Ring / Silver / RD11" at Rs
  // 2,000) -- confirmed live to produce a Rs 1,600 surcharge that
  // actually charged Rs 2,000 (rounds to the nearest whole multiple of
  // that variant's real price, so anything not a clean multiple of it is
  // wrong -- the original version of this bug was off by orders of
  // magnitude, e.g. Rs 32 crore for a Rs 1,60,000 surcharge). Type
  // "Utility" never matches getCustomisationVariant()'s real lookup
...
export async function buildGemstoneCustomisationMatrix(admin, rates = {}) {
  ...
  try {
    ...
    if (variantsNodes.length > 0) {
      ...
      // The fallback used for custom-design (uploaded photo) surcharges
      // in submitCustomizerBundle() MUST be the dedicated Rs-1 "Utility /
      // Unit" variant added in generateAllCustomisationVariants() above
      // -- NOT matrixArray[0] (an arbitrary REAL design, whatever
      // happened to sort first), which is what silently caused a Rs
      // 1,600 surcharge to charge Rs 2,000 (and, before that fallback's
      // own rounding fix, a Rs 1,60,000 surcharge to charge Rs 32 crore).
      // Falling back to matrixArray[0] only if the Rs-1 variant is
      // somehow missing (e.g. an old snippet regenerated by code that
      // predates this variant existing) keeps this from ever being
      // blank, while making clear that path is degraded, not the norm.
      const helperVariant = matrixArray.find((v) => v.type === "Utility" && v.metal === "Unit");
      const defaultHelperVariantId = helperVariant?.id || matrixArray[0]?.id || "";

      const snippetContent = `<script>
  window._shubhCustomisationMatrix = ${JSON.stringify(matrixArray)};
  window._shubhDefaultHelperVariantId = "${defaultHelperVariantId}";
</script>`;
  ...
  const preview = variantsNodes
    ...
    // Same exclusion as fetchCustomisationVariantsPreview -- the internal
    // Rs-1 "Utility / Unit" fallback variant isn't a real design.
    .filter((v) => !(v.type === "Utility" && v.metal === "Unit"))
...
export async function fetchCustomisationVariantsPreview(admin) {
  ...
  return nodes
    ...
    // The internal "Utility / Unit" Rs-1 variant (see
    // generateAllCustomisationVariants) isn't a real design a merchant
    // would recognize -- excluded here so it doesn't show up as a
    // confusing Rs 1 row in the app's own "Live Design Prices" table.
    .filter((v) => !(v.type === "Utility" && v.metal === "Unit"))
    .sort((a, b) => a.type.localeCompare(b.type) || a.metal.localeCompare(b.metal) || a.design.localeCompare(b.design));
}
